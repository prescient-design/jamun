...
def test_basic_functionality():
    ...
    print(f"Input parameters: N={N}, subsample_rate={subsample_rate}, "
    ...
    lagged_indices = get_subsampled_indices(
        N, subsample_rate, total_lag_time, lag_subsample_rate
    )
    ...
    for i, lagged in enumerate(lagged_indices):
        ...
        assert np.array_equal(lagged, expected_lagged), \
            f"For index {i}, expected {expected_lagged}, got {lagged}"
...
def test_edge_cases():
    ...
    assert np.array_equal(subsampled_indices, np.array([0])), \
        f"Expected [0], got {subsampled_indices}"
    assert np.array_equal(lagged_indices[0], np.array([0])), \
        f"Expected [0], got {lagged_indices[0]}"
...
def test_large_numbers():
    ...
    for i, lagged in enumerate(lagged_indices):
        ...
        print(f"Individual values and their types:")
        for j, val in enumerate(lagged):
            print(f"  Index {j}: value={val}, type={type(val)}")
        
        assert len(lagged) == total_lag_time, \
            f"Expected lagged indices length {total_lag_time}, got {len(lagged)}"
        ...
        for j, val in enumerate(lagged):
            ...
            assert val >= 0, \
                f"Found negative value at index {j}: {val}"
