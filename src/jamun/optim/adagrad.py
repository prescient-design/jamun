...
class Adagrad(Optimizer):
    ...
    Args:
        ...
        eta (float, optional): an experimental feature used for learning rate correction (similar to madgrad)
...
def _single_tensor_adagrad(
    ...
    *,
    ...
    for param, grad, state_sum, step_t in zip(params, grads, state_sums, state_steps):
        ...
        lr_k = lr * math.pow(step + 1, eta)  # [saeed] experimental feature!
        clr = lr_k / (1 + (step - 1) * lr_decay)

        if grad.is_sparse:
            grad = grad.coalesce()  # the update is non-linear so indices must be unique
            grad_indices = grad._indices()
            grad_values = grad._values()
            size = grad.size()  # noqa: F841
            ...
            std[std == 0] = float("inf")  # [saeed] to handle eps=0
