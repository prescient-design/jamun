...
def check_compatibility_with_config(checkpoint_path: str, config_path: str, module_path: str | None = None):
                    ...
                    with hydra.initialize_config_dir(config_dir=str(Path(config_path).parent.absolute())):
                        ...
                        if isinstance(cfg, DictConfig):
                            ...
                            if isinstance(target_model, compatibility["model_class"]):
                                ...
                                print(
                                    f"⚠️  Config model class ({type(target_model).__name__}) differs from checkpoint ({compatibility['model_class'].__name__})"
                                )
                except Exception as e:
...
def extract_and_save_module(checkpoint_path: str, module_path: str, output_path: str):
...
def main():
    ...
    parser.add_argument("--checkpoint_type", type=str, default="best_so_far", help="Type of checkpoint to load")

    # Inspect command arguments
    parser.add_argument("--max_depth", type=int, default=2, help="Maximum depth for structure inspection")
    ...
    if args.checkpoint:
        ...
        checkpoint_path = find_checkpoint(wandb_train_run_path=args.wandb_run, checkpoint_type=args.checkpoint_type)
    else:
