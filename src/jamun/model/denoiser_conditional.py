...
class DenoiserWithInputAttr(Denoiser):
    """
    Extension of Denoiser that works with E3ConvConditionalWithInputAttr.
    
    This subclass modifies the xhat_normalized method to extract spatiotemporal
    features from the conditioner and pass them as input_attr to the architecture.
    """
    ...
    def __init__(
        ...
        """
        Initialize DenoiserWithInputAttr.
        
        Args:
            All arguments are the same as the parent Denoiser class.
            The conditioner should be a SpatioTemporalConditioner that provides
            features that can be used as input_attr.
        """
        super().__init__(
            arch=arch,
            optim=optim,
            ...
            add_fixed_noise=add_fixed_noise,
            add_fixed_ones=add_fixed_ones,
            align_noisy_input_during_training=align_noisy_input_during_training,
            align_noisy_input_during_evaluation=align_noisy_input_during_evaluation,
            mean_center=mean_center,
            mirror_augmentation_rate=mirror_augmentation_rate,
            bond_loss_coefficient=bond_loss_coefficient,
        ...
        # Verify that we have a conditioner (required for this subclass)
        if self.conditioning_module is None:
            raise ValueError(
                "DenoiserWithInputAttr requires a conditioner to provide input attributes"
            )
    ...
    def xhat_normalized(
        ...
        This version extracts spatiotemporal features from the conditioner and
        passes them as input_attr to E3ConvConditionalWithInputAttr.
        """
        sigma = torch.as_tensor(sigma).to(y.pos)
        D = y.pos.shape[-1]
        ...
        # Extract spatiotemporal features from conditioner
        with torch.cuda.nvtx.range("conditioning"): 
            conditioned_structures = self.conditioner(y_scaled)
            # print(f"Conditioner is working, number of conditioned structures: {len(conditioned_structures)}")
        
        # Extract spatiotemporal features as input_attr
        with torch.cuda.nvtx.range("extract_input_attr"):
            # The conditioner should provide spatiotemporal features
            # For SpatioTemporalConditioner, we need to extract the features from the model
            if hasattr(self.conditioning_module, 'spatiotemporal_model'):
                # Get spatiotemporal features by calling the model with return_spatial_features=True
                spatiotemporal_result = self.conditioning_module.spatiotemporal_model(
                    y_scaled, 
                    unsqueeze_trailing(torch.tensor(self.conditioning_module.c_noise, device=y.pos.device), 1)
                )
                # Extract spatial features as input_attr
                if isinstance(spatiotemporal_result, dict):
                    input_attr = spatiotemporal_result.get('spatial_features', None)
                else:
                    # If not dict, assume the result is the spatial features
                    input_attr = spatiotemporal_result
            else:
                # Fallback: use zeros with correct shape and irreps
                # This allows the model to work even without a proper spatiotemporal conditioner
                py_logger = logging.getLogger("jamun")
                py_logger.warning("No spatiotemporal_model found in conditioner, using zero input_attr")
                # Assume input_attr_irreps is "3x1e" (9D vector) as specified in config
                input_attr = torch.zeros(y.pos.shape[0], 9, device=y.pos.device, dtype=y.pos.dtype)

        # Call the architecture with input_attr
        with torch.cuda.nvtx.range("g"):    
            g_pred = self.g(
                torch.cat([*conditioned_structures], dim=-1),  # Concatenated positions as first arg
                topology=y_scaled,
                c_noise=c_noise, 
                effective_radial_cutoff=radial_cutoff,
                input_attr=input_attr  # Pass spatiotemporal features as input_attr
            )
