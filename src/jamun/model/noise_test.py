...
def run_test():
    ...
    data1 = Data(
        ...
        hidden_state=[torch.randn(num_nodes, 4), torch.randn(num_nodes, 8)]
    )
    # another data object
    data2 = Data(
        pos=torch.randn(num_nodes + 2, 3),
        hidden_state=[torch.randn(num_nodes + 2, 4), torch.randn(num_nodes + 2, 8)]
    )
