...
class SpatioTemporalConditioner(pl.LightningModule):
    ...
    def forward(self, y: torch_geometric.data.Batch) -> list[torch.Tensor]:
        ...
        sigma = unsqueeze_trailing(
            sigma, 1
        )  # actually this is correct, but this is bad variable naming. the positional e3conv will take a c_noise, so this is right, but it is not right to call it sigma.
