...
def get_ala_ala_data(num_frames=20, total_lag_time=5):
    ...
    if data_path is None:
        ...
        possible_paths = ["/data/bucket/kleinhej/", "/data2/sules/", "/path/to/data/"]
    ...
    if data_path is None:
        raise ValueError(
            "JAMUN_DATA_PATH not set and cannot find data. Please set JAMUN_DATA_PATH environment variable."
        )
...
def create_test_denoiser_spiked(total_lag_time=5):
    ...
    arch = functools.partial(
        ...
        output_head_factory=functools.partial(e3tools.nn.EquivariantMLP, irreps_hidden_list=["32x0e + 8x1e"]),
    )
...
def test_noise_and_denoise():
    ...
    try:
        ...
        print(
            f"   Sample hidden states: {len(sample.hidden_state) if hasattr(sample, 'hidden_state') and sample.hidden_state else 0}"
        )
    ...
    try:
        ...
        print("✅ Successfully created DenoiserSpiked model")
    ...
    try:
        ...
        for sigma in sigma_values:
            ...
            with torch.no_grad():
                x_target, xhat, y_noisy = denoiser.noise_and_denoise(batch, sigma=sigma, align_noisy_input=True)

            print("     ✅ noise_and_denoise completed successfully")
            ...
            # Test conditioning
            print("     Testing conditioner...")
            print(
                f"     y_noisy has hidden_state: {hasattr(y_noisy, 'hidden_state') and y_noisy.hidden_state is not None}"
            )
            ...
            if x_target is not None:
                ...
                print(
                    f"     x_target has hidden_state: {hasattr(x_target, 'hidden_state') and x_target.hidden_state is not None}"
                )
            ...
            if len(conditioned_structures) > 0:
                ...
                if torch.allclose(last_structure, clean_structure, atol=1e-6):
                    print("     ✅ Last conditioned structure matches x_clean.pos")
                else:
