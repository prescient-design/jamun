...
from convert_spatiotemporal import (
    calculate_temporal_positions,
    spatial_to_temporal_graphs,
    temporal_to_spatial_graphs
)

# Import node attribute conversion functions  
from pooling import (
    spatial_to_temporal_node_attr,
    temporal_to_spatial_node_attr,
    temporal_to_spatial_node_attr_mean,
    SpatialTemporalToTemporalNodeAttr,
    TemporalToSpatialNodeAttrMean
)
...
import e3tools
from e3nn import o3
from helpers import create_e3conv_network, get_e3conv_output_irreps, apply_e3conv_to_positions
...
def test_temporal_conversion(batch, graph_type="fan"):
    ...
    print(f"Original spatial batch:")
    print(f"  - pos shape: {batch.pos.shape}")
    print(f"  - hidden_state length: {len(batch.hidden_state) if hasattr(batch, 'hidden_state') and batch.hidden_state else 0}")
    ...
    print(f"  - example edge_index for first temporal graph:")
...
print(f"Temporal batch device verification:")
...
with torch.no_grad():
    ...
    node_attr_current = spatial_e3conv(batch.pos, topology, batch.batch, \
                                       num_graphs=batch.num_graphs,\
                                       c_noise=sigma,\
                                        effective_radial_cutoff=0.05).unsqueeze(1)
    ...
    if hasattr(batch, 'hidden_state') and batch.hidden_state:
        for hidden_pos in batch.hidden_state:
            node_attr_hidden = node_attr_current = spatial_e3conv(hidden_pos, topology, batch.batch, \
                                       num_graphs=batch.num_graphs,\
                                       c_noise=sigma,\
                                        effective_radial_cutoff=0.05).unsqueeze(1)
...
def test_e3_transformer(batch, temporal_batch, spatial_node_attr_all_temporal, device):
    ...
    with torch.no_grad():
        try:
            transformer_output = transformer(temporal_node_attr, temporal_batch, effective_radial_cutoff, temporal_cutoff)
...
assert spatial_features_pooled.shape[0] == batch.pos.shape[0], f"Spatial node count mismatch: {spatial_features_pooled.shape[0]} vs {batch.pos.shape[0]}"
