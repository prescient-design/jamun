...
try:
    ...
    if not hasattr(cfg.model, "conditioner"):
        ...
        cfg.model.conditioner._target_ = (
            "scratch.conditioners.SelfConditioner"  # Use the SelfConditioner from scratch.conditioners
        )
...
if (
    loggers_list
    and isinstance(loggers_list[0], pl.loggers.WandbLogger)
    and hasattr(loggers_list[0], "experiment")
    and loggers_list[0].experiment
    and hasattr(loggers_list[0].experiment, "dir")
    and loggers_list[0].experiment.dir
):
    ...
    if not loggers_list:
        ...
        py_logger.info(
            f"First logger is not a suitable WandB logger. Defaulting checkpoint directory: {final_checkpoint_dir}"
        )
...
if wandb_logger_instance and hasattr(wandb_logger_instance, "experiment") and wandb_logger_instance.experiment:
    py_logger.info(
        f"WandbLogger experiment active (run_id: {wandb_logger_instance.experiment.id}). Logging final script config to wandb."
    )
    ...
    if cfg.get("logger") and cfg.logger.get("wandb"):
        py_logger.warning(
            "WandbLogger was configured but not found or experiment not active at script end. Final script config not logged to wandb.config."
        )
...
if cfg.get("logger") and cfg.logger.get("wandb") and cfg.logger.wandb.get("save_dir"):
    ...
    final_config_output_dir = os.path.join(
        os.getcwd(), "outputs", cfg.get("task_name", "unknown_task"), cfg.get("run_key", "unknown_run")
    )
...
if final_config_output_dir:
    ...
    py_logger.warning(
        "Could not determine a definitive output directory for final_resolved_script_config.yaml. Not saving locally."
    )
