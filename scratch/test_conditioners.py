...
from jamun.model.conditioners.conditioners import MeanConditioner, PositionConditioner, SelfConditioner
...
def print_tensor_summary(tensor, name, max_elements=6):
    ...
    if tensor.numel() <= max_elements:
        ...
        flat = tensor.flatten()
        print(
            f"{name} (shape {tensor.shape}): [{flat[0]:.6f}, {flat[1]:.6f}, {flat[2]:.6f}, ..., {flat[-3]:.6f}, {flat[-2]:.6f}, {flat[-1]:.6f}]"
        )
...
def print_batch_details(batch, batch_name):
    ...
    if pos_mean_magnitude < 1e-6:
        print("✅ Input position is mean centered")
    else:
        print("❌ Input position is NOT mean centered")
    ...
    try:
        ...
        for i, structure in enumerate(conditioned_structures):
            ...
            if conditioner.__class__.__name__ == "PositionConditioner":
                ...
                print(
                    f"Mean of structure {i}: [{structure_mean[0]:.6f}, {structure_mean[1]:.6f}, {structure_mean[2]:.6f}]"
                )
...
def main():
    """Main test function."""
    print(
        "Testing Conditioners: SelfConditioner, PositionConditioner, MeanConditioner with 3 total structures (2 hidden states)"
    )
    print("=" * 70)
    ...
    try:
        ...
        print("✅ Created conditioners")
    except Exception as e:
        print(f"❌ ERROR: Failed to create conditioners: {e}")
        return False

    # Test 1: SelfConditioner on MDtrajDataset
    result1 = test_conditioner_detailed(self_conditioner, mdtraj_batch, "TEST 1: SelfConditioner on MDtrajDataset")
    if result1 is False:
        return False
    ...
    result2 = test_conditioner_detailed(
        self_conditioner, repeated_batch, "TEST 2: SelfConditioner on RepeatedPositionDataset"
    )
    if result2 is False:
        return False
    ...
    result3 = test_conditioner_detailed(
        position_conditioner, mdtraj_batch, "TEST 3: PositionConditioner on MDtrajDataset"
    )
    if result3 is False:
        return False
    ...
    result4 = test_conditioner_detailed(
        position_conditioner, repeated_batch, "TEST 4: PositionConditioner on RepeatedPositionDataset"
    )
    if result4 is False:
        return False
    ...
    result5 = test_conditioner_detailed(mean_conditioner, mdtraj_batch, "TEST 5: MeanConditioner on MDtrajDataset")
    if result5 is False:
        return False
    ...
    result6 = test_conditioner_detailed(
        mean_conditioner, repeated_batch, "TEST 6: MeanConditioner on RepeatedPositionDataset"
    )
    if result6 is False:
        return False
