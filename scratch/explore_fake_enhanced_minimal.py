...
def quick_exploration():
    ...
    py_logger.info("=" * 60)
    py_logger.info(
        f"Parameters: subsample={subsample_rate}, total_lag_time={total_lag_time}, lag_subsample_rate={lag_subsample_rate}"
    )
    py_logger.info("=" * 60)
    ...
    try:
        ...
        if datasets:
            ...
            for i, dataset in enumerate(datasets):
                ...
                if i == 0:
                    ...
                    if hasattr(sample, "hidden_state") and sample.hidden_state:
                        ...
                        py_logger.info("  No hidden state found")
            ...
            py_logger.info(
                f"With subsample={subsample_rate}, each dataset gives ~{total_frames / len(datasets):.0f} trajectories"
            )
...
def test_different_subsample_rates():
    ...
    for params in test_params:
        py_logger.info(f"\nTesting: {params['desc']}")
        py_logger.info(
            f"  subsample={params['subsample']}, total_lag_time={params['total_lag_time']}, lag_subsample_rate={params['lag_subsample_rate']}"
        )
        ...
        try:
            ...
            if datasets:
                ...
                py_logger.info("  -> No datasets found")
