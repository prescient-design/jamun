...
pdb_path_conditional = (
    "/data2/sules/jamun-conditional-runs/outputs/sample/dev/runs/2025-08-13_20-22-36/sampler/ALA_ALA/topology.pdb"
)
...
pdb_path_unconditional = (
    "/data2/sules/jamun-conditional-runs//outputs/sample/dev/runs/2025-08-19_18-56-30/sampler/ALA_ALA/topology.pdb"
)
...
cumulants_conditional = np.array(
    [
        np.sum(issues_array_conditional[:i]) / np.sum(issues_array_conditional)
        for i in range(issues_array_conditional.shape[0])
    ]
)
...
cumulants_unconditional = np.array(
    [
        np.sum(issues_array_unconditional[:i]) / np.sum(issues_array_unconditional)
        for i in range(issues_array_unconditional.shape[0])
    ]
)

issues_array_md = np.array(bond_length_issues_md)
total_issues_md = np.sum(issues_array_md)
cumulants_md = np.array(
    [np.sum(issues_array_md[:i]) / np.sum(issues_array_md) for i in range(issues_array_md.shape[0])]
)
...
plt.subplot(1, 2, 1)
plt.hist(
    issues_array_conditional, bins=10, alpha=0.7, range=(0.0, 1.0), edgecolor="black", color="blue", label="KALA-JAMUN"
)
plt.hist(
    issues_array_unconditional, bins=10, alpha=0.7, range=(0.0, 1.0), edgecolor="black", color="red", label="JAMUN"
)
plt.hist(
    issues_array_md,
    bins=10,
    ...
    edgecolor="black",
    color="green",
    label="Reference MD Trajectory issues",
)
plt.legend()
...
plt.subplot(1, 2, 2)
plt.plot(
    np.linspace(0, 1, issues_array_conditional.shape[0]),
    cumulants_conditional,
    alpha=0.5,
    color="blue",
    label="KALA-JAMUN",
    linewidth=5,
...
plt.plot(
    np.linspace(0, 1, issues_array_unconditional.shape[0]),
    cumulants_unconditional,
    alpha=0.5,
    color="red",
    label="JAMUN",
    linewidth=5,
...
plt.plot(
    np.linspace(0, 1, issues_array_md.shape[0]),
    cumulants_md,
    alpha=0.5,
    color="green",
    label="Reference MD Trajectory",
    linewidth=5,
)
plt.legend()
