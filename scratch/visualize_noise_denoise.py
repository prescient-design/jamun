...
pdb.set_trace()
from jamun.data import MDtrajDataset, parse_datasets_from_directory
...
def load_model_from_wandb(wandb_run_path: str, checkpoint_type: str = "last", checkpoint_path: str = None):
    ...
    checkpoint_path_wandb = find_checkpoint(wandb_train_run_path=wandb_run_path, checkpoint_type=checkpoint_type)
...
def process_trajectory(model, dataset: MDtrajDataset, sigma: float = 0.04):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=32, shuffle=False, collate_fn=torch_geometric.data.Batch.from_data_list
    )
...
def create_ramachandran_plot(clean_traj, noisy_traj, denoised_traj, title: str, save_path: str):
    ...
    trajs = {"x": clean_traj, "y": noisy_traj, "xhat": denoised_traj}
...
def main():
    ...
    model.eval()
    model.to("cuda:0")
    print("Model loaded and moved to cuda:0")
    ...
    if all_clean_samples:
        ...
        create_ramachandran_plot(
            concat_clean_traj, concat_noisy_traj, concat_denoised_traj, "Concatenated Trajectories", concat_plot_path
        )
