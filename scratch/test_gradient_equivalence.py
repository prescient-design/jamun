...
def compare_gradients(grad1, grad2, tolerance=1e-3):
    ...
    for name in grad1.keys():
        ...
        print(f"{name:30s}: max_rel_diff = {max_rel_diff_param:.6f}, norm_ratio = {g1.norm().item()/g2.norm().item():.6f}")
...
def test_gradient_equivalence(cfg):
    ...
    with torch.no_grad():
        ...
        x_target_manual, xhat_manual, y_manual = model_manual.noise_and_denoise(batch_manual, sigma_manual, align_noisy_input=True)
