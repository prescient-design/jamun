    ...
    for split in ["train", "val"]:
        ...
        for xtc_file in test_files:
            ...
            try:
                ...
                traj = md.load(xtc_path, top=pdb_path)
                logger.info(
                    f"✅ {split}: Successfully loaded {xtc_file} + {pdb_file} "
...
def reorganize_with_grid_split(grid_codes: list[str], strategy_config: dict):
    ...
    copy_files_for_grid_split(
        SOURCE_DIR, os.path.join(target_dir, "train"), train_codes, TRAJECTORY_CODES, SINGLE_PDB_FILE, "TRAIN"
    )

    # Copy files for val split
    logger.info("Copying files for val split...")
    copy_files_for_grid_split(
        SOURCE_DIR, os.path.join(target_dir, "val"), val_codes, TRAJECTORY_CODES, SINGLE_PDB_FILE, "VAL"
    )
...
def reorganize_with_trajectory_split(grid_codes: list[str], strategy_config: dict):
    ...
    copy_files_for_trajectory_split(
        SOURCE_DIR, os.path.join(target_dir, "train"), grid_codes, train_trajectories, SINGLE_PDB_FILE, "TRAIN"
    )

    # Copy files for val split (all grids, last trajectory)
    logger.info("Copying files for val split...")
    copy_files_for_trajectory_split(
        SOURCE_DIR, os.path.join(target_dir, "val"), grid_codes, val_trajectories, SINGLE_PDB_FILE, "VAL"
    )
...
def copy_files_for_state_split(
    ...
    logger.info("State split completed:")
...
def reorganize_with_state_split(grid_codes: list[str], strategy_config: dict):
    ...
    copy_files_for_state_split(
        SOURCE_DIR, target_dir, grid_codes, trajectory_codes, SINGLE_PDB_FILE, phi_range, psi_range
    )
