...
from jamun.utils import unsqueeze_trailing, mean_center
...
def emulate_xhat_normalized_scaling(batch, sigma: float, average_squared_distance: float = 0.332):
    ...
    if hasattr(batch, "hidden_state") and batch.hidden_state is not None:
        ...
        for i, positions in enumerate(batch.hidden_state):
            ...
            print(f"Hidden state {i} - Original mean: {positions.mean():.6f}, Scaled mean: {scaled_positions.mean():.6f}")
...
def test_denoised_conditioner_with_scaling():
    ...
    try:
        ...
        elif hasattr(conditioner.pretrained_denoiser, 'hparams') and hasattr(conditioner.pretrained_denoiser.hparams, 'average_squared_distance'):
        ...
        print(f"✓ Conditioner forward pass completed")
        print(f"  Returned {len(conditioned_structures)} structures")
        print(f"  Expected N_structures: {N_structures}")
        
        # Verify output structure
        assert len(conditioned_structures) == N_structures, f"Expected {N_structures} structures, got {len(conditioned_structures)}"
        
        for i, structure in enumerate(conditioned_structures):
            assert structure.shape == scaled_batch.pos.shape, f"Structure {i} has wrong shape: {structure.shape} vs {scaled_batch.pos.shape}"
            print(f"  Structure {i}: shape {structure.shape}")
        
        # Check that first structure is the scaled current position
        assert torch.allclose(conditioned_structures[0], scaled_batch.pos), "First structure should be scaled current position"
        ...
        if hasattr(x_clean, "hidden_state") and x_clean.hidden_state and len(conditioned_structures) > 1:
            ...
            for i in range(1, len(conditioned_structures)):  # Skip first structure (current position)
                ...
                if hidden_idx < len(x_clean.hidden_state):
                    ...
                    if improvement > 0:
                        print(f"    ✓ DENOISING SUCCESSFUL (RMSE reduced)")
                    else:
                        print(f"    ❌ DENOISING FAILED (RMSE increased)")
                    
                    # Verify denoised is different from both noisy and original
                    assert not torch.allclose(denoised_structure, noisy_hidden, atol=1e-4), f"Denoised structure {i} should be different from noisy"
                    assert not torch.allclose(denoised_structure, scaled_batch.pos, atol=1e-4), f"Denoised structure {i} should be different from current position"
        ...
        if denoising_improvements:
            ...
            if success_rate >= 80:  # Require at least 80% success rate
                print(f"  ✅ DENOISING QUALITY: EXCELLENT (≥80% success)")
            elif success_rate >= 60:
                print(f"  ⚠️ DENOISING QUALITY: GOOD (≥60% success)")
            elif success_rate >= 40:
                print(f"  ⚠️ DENOISING QUALITY: MODERATE (≥40% success)")
            else:
                print(f"  ❌ DENOISING QUALITY: POOR (<40% success)")
            ...
            print(f"  ✓ At least some denoising improvement verified")
