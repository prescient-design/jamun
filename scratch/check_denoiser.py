...
try:
    ...
    if not hasattr(cfg.model, "average_squared_distance") or cfg.model.average_squared_distance is None:
        ...
        average_squared_distance = compute_average_squared_distance_from_datasets(
            datasets["test"], cfg.model.max_radius
        )
    ...
    if not hasattr(cfg.model, "conditioner"):
        ...
        cfg.model.conditioner._target_ = (
            "scratch.conditioners.PositionConditioner"  # Use the PositionConditioner from scratch.conditioners
        )
...
if (
    not hasattr(data_batch, "hidden_state")
    or not isinstance(data_batch.hidden_state, list)
    or len(data_batch.hidden_state) != denoiser_to_test.g._orig_mod.N_structures - 1
):  # Use _orig_mod to access N_structures if g is compiled
...
try:
    ...
    xhat1, y_processed1 = denoiser_to_test.noise_and_denoise(original_x.clone(), sigma, align_noisy_input=True)
    ...
    assert not torch.allclose(y_processed1.pos, original_x.pos), (
        "y_processed1.pos should be different from original x.pos"
    )
    ...
    if hasattr(original_x, "hidden_state") and original_x.hidden_state:
        assert hasattr(y_processed1, "hidden_state") and len(y_processed1.hidden_state) == len(
            original_x.hidden_state
        ), "y_processed1.hidden_state length mismatch"
        for i in range(len(original_x.hidden_state)):
            assert not torch.allclose(y_processed1.hidden_state[i], original_x.hidden_state[i]), (
                f"y_processed1.hidden_state[{i}] should be different"
            )
    ...
    if hasattr(y_processed1, "hidden_state") and y_processed1.hidden_state:
        assert hasattr(xhat1, "hidden_state") and len(xhat1.hidden_state) == len(y_processed1.hidden_state), (
            "xhat1.hidden_state length mismatch with y_processed1"
        )
...
try:
    ...
    if (
        not hasattr(train_batch, "hidden_state")
        or not isinstance(train_batch.hidden_state, list)
        or len(train_batch.hidden_state) != denoiser_to_test.g._orig_mod.N_structures - 1
    ):
        n_structures = denoiser_to_test.g._orig_mod.N_structures
