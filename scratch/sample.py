...
def get_initial_graphs(
    ...
    for dataset in datasets:
        ...
        for index in random_indices:
            ...
            # Ensure graph has hidden state
            if not hasattr(init_graph, 'hidden_state'):
                init_graph.hidden_state = torch.zeros(init_graph.num_nodes, 1)
...
def run(cfg):
    ...
    dist_log(f"Found checkpoint at: {checkpoint_path}")

    # Load checkpoint and create model
    checkpoint = torch.load(checkpoint_path, map_location='cpu')
    dist_log("Loaded checkpoint successfully")

    # Create model with conditioner
    ...
    if not hasattr(cfg.model, 'conditioner'):
        cfg.model.conditioner = OmegaConf.create({
            "_target_": "scratch.conditioners.SelfConditioner",
            "hidden_dim": 1  # Adjust based on your needs
        })
    
    model = hydra.utils.instantiate(cfg.model)
    dist_log("Created model instance")

    # Load state dict into model
    model.load_state_dict(checkpoint['state_dict'])
    dist_log("Loaded state dict into model")
    
    # Breakpoint 1: After model instantiation and loading
    pdb.set_trace()  # Check model architecture and weights
    ...
    # Breakpoint 2: After dataset loading
    pdb.set_trace()  # Check dataset and initial graphs
    ...
    # Run test-time adaptation, if specified
    if finetuning_cfg := cfg.get("finetune_on_init"):
        ...
        # Breakpoint 3: Before finetuning
        pdb.set_trace()  # Check model state before finetuning
    ...
    # Breakpoint 4: Before sampling
    pdb.set_trace()  # Check final model state before sampling
